...
unsigned int min_perc = 0, max_perc = 100, by_time, old_times, max_size, is_zip;
unsigned int cut_tasks, break_states, keep_events = 0;
unsigned long long time_min, time_max, total_time, first_record_time, last_record_time;
...
#if defined(ParaverMode)
	extern struct utilities_options exec_options;
#else
...
void read_cutter_params()
{
	...
	if(exec_options.tasks_list != NULL)
	{
		cut_tasks = 1;
		int j = 0;
...
void read_args(int argc, char *argv[])
{
	...
	if(argc < 5)
	{
		...
		printf("  -p  <percent_min> <percent_max>\tCut traces by percentage of time\n");
		printf("  -original_time\t\t\tKeep times of the original trace.\n\t\t\t\t\tIf this flag is not specified, the\n\t\t\t\t\tnew cutted trace will start at time 0\n");
		printf("  -tasks <id_1[,id_2...]]>\t\tPut info only for the identifiers\n\t\t\t\t\tspecified in the list where every id\n\t\t\t\t\tcould be a task_id or a range of tasks\n\t\t\t\t\t[task_id_1-task_id_2]\n");
		printf("  -not_break_states\t\t\tWhen cutting without original time, \n\t\t\t\t\tdon't split states due to the cut\n");
		printf("  -keep_boundary_events\t\t\tKeep the events outside the cutting zone\n\t\t\t\t\tof the bursts delimiting the cut\n");
		printf("  -remove_first_state\t\t\tOmmit states which don't fit entirely \n\t\t\t\t\tin the beginnig of the cut\n");
		printf("  -remove_last_state\t\t\tOmmit states which don't fit entirely \n\t\t\t\t\tin the end of the cut\n");
...
void proces_cutter_header(char *header, char *trace_in_name, char *trace_out_name)
{
	...
	if(strstr(word, "_ns"))
	{
		...
		if(!old_times) current_size+=fprintf(outfile, "%lld_ns:", total_time);
		else current_size+=fprintf(outfile, "%s_ns:",word);
	}
	else
	{
		trace_time = atoll(word);
...
#if !defined(ParaverMode)
	if((word = strrchr(word,','))==NULL) return;
#else
	word = strrchr(word,',');
#endif
...
void update_queue(int appl, int task, int thread, unsigned long long type, unsigned long long value)
{
...
void shift_trace_to_zero(char *nameIn, char *nameOut)
{
	...
	int first_record, cpu, appl, task, thread, state, cpu_2, appl_2, task_2, thread_2;
	char *trace_header;
...
void copy_file(char *in, char *out)
{
	...
	if((fileIn = fopen(in,"r")) == NULL) return;
	fileOut = fopen(out,"w");
...
int cutter_main(int argc, char *argv[])
{
		...
		{
	#else
		void cutter(char *trace_in, char *trace_out)
		{
	#endif
#endif
	...
	unsigned long long type, value, time_1, time_2, time_3, time_4, end_time = 0;
	int i,j,k,end_line;
	...
	min_perc = 0; max_perc = 100;keep_events = 0; total_cutter_iters = 0;
	by_time = 0; old_times = 0; max_size = 0;
	cut_tasks = 0; break_states = 1; is_zip = 0;
	init_task_counter = 0; usefull_tasks = 0; first_time_caught = 0;
	num_tasks = 0; current_size = 0;
	...
	else
	{
		...
		if( envptr != NULL )
		{
			...
			printf("\nUpdating log file...");
			if(by_time)
				fprintf(log_file,"%s.prv:\tFrom  %s.prv\tMin_time: %lld\t Max_time: %lld\n\n",argv[2], argv[1], time_min, time_max);
			else
				fprintf(log_file,"%s.prv:\tFrom  %s.prv\tMin_percentage: %lld\t Max_percentage: %lld\n\n",argv[2], argv[1], min_perc, max_perc);
	...
	while( !end_parsing )
	{
		...
		switch(line[0])
		{
			...
			case '2':
				...
				*/
				if(tasks[appl-1][task-1][thread-1] != NULL && time_1 > tasks[appl-1][task-1][thread-1]->last_time && time_1 > time_max) break;
				...
				if((time_1 >= time_min && time_1 <= time_max) || (time_1 < time_min && tasks[appl-1][task-1][thread-1]!= NULL && tasks[appl-1][task-1][thread-1]->last_time >= time_min && keep_events) || (time_1 > time_max && tasks[appl-1][task-1][thread-1]!=NULL && tasks[appl-1][task-1][thread-1]->last_time >= time_1 && keep_events))
				{
					if(!old_times) time_1 = time_1 - time_min;

					if(time_1 > last_record_time) last_record_time = time_1;
			...
			case '4':
				...
				if(old_times) current_size+=fprintf(outfile,"%d:%d:%d:%d:%d:%lld:%s\n",id,cpu,appl,task,thread,time_1,line);
				else
				{
