...
SYNOPSIS
  cgprof [-h] [-T dev] [-g gmon_data] [gprof_file] [-o file]
...
OPTIONS
  ...
  -T dev  This option sets the output format to dev. Possible values of
	  ...
	  ps   Postscript output, using dot(1) processor. The drawing will be
	       sized to fill one landscape page. Default file graph.ps, to change
	       use -o "file" parameter.
  -o file File which is going to print -Tps.
...
# Some locales use a decimal separator different from "." (for instance, catalan locale)
# awk generates the decimal separator defined by the locale, but dot expects "."
# Using en_US locale ensures "." separator
#
# Added by Enric Morancho, Oct-2007
# Modified by Jeremies Marin Oct-2010
...
case $dev in
	X)  post="dotty -";;
	ps) post="dot -Tps -Gsize=\"11,17\" -Grotate=90 -o $file";;
	*)  post="cat"
esac
