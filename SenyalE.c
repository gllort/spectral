...
float Minim(long long int p, long long int *t, long long int *delta, float *semanticvalue, int i, long long int llargint) {
...
void Passadamin(long long int *t, long long int *delta, float *semanticvalue, int i, long long int *B1, float *B2, long long int longint) {
...
for(j=0; j<3*i-1; j++) {
	B2[j]=Minim(B1[j], t, delta, semanticvalue, i, longint);
}
}

int SenyalE(long long int *t, long long int *delta, float *semanticvalue, int i, long long int longint, long long int *t2, long long int *delta2, float *semanticvalue2) {
	...
	Passadamin(t, delta, semanticvalue, i, B1, B2, longint);
	...
	for(j=1; j<=k; j++) {
		...
		P2[2*j-2]=temp2[j-1];
		P1[2*j-1]=temp1[j-1];
		P2[2*j-1]=Minim(temp1[j-1]+1, t, delta, semanticvalue, i, longint);
	}
	sizeP=2*k;
	sizeM=2*k-1;
fprintf(stderr, "SenyalE sizeP %d\n", sizeP);
	j=0;
	k=0;
	...
	/* for (count = 0; count < k; count++) {
		printf("%lld %lld %f\n", t2[count], delta2[count], semanticvalue2[count]);
  	}  */
...
{ 
  ...
  i=0;
  while (fscanf(fp, "%u%u%f", &a, &b, &c)==3) {
  	i++;	
  }
  /*printf("Numero de files: %u \n", i);*/
  ...
  sizeM=SenyalE(t, delta, semanticvalue, i, longme, t2, delta2, semanticvalue2);
